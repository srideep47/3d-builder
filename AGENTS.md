...
- **Two Python environments, never mixed**: main `.venv` (light deps) and
  ...
  Start the service with `scripts/start-img3d.ps1 [tripo_sr|trellis]` before
  builds that contain `image_to_3d` parts (mock backend runs in the main
  env). The trellis backend is the exception that needs NO Python deps: it
  drives the trellis.cpp server (TRELLIS.2-4B as GGUF) — install once with
  `scripts/setup-trellis-cpp.ps1` (~10 GB at the default q8 tier, under
  gitignored `models/trellis/`), then `start-img3d.ps1 trellis`; the
  backend spawns the server itself (adopting an already-healthy one on
  127.0.0.1:8712, terminating only what it spawned) and fails closed until
  the full 10-file GGUF set + binary are present.
